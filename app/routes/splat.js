...
exports.editMovie = function(req, res) {
    Movie.findById(req.params.id, function(findErr, movie) {
        if (findErr) {
            res.status(500).send('Server is unable to process movie update; ' + 'please try again later ' + findErr.message);
        } else if (!movie) {
...
exports.uploadVideo = function(req, res) {
    ...
    fs.rename(filePath, newPath, function(err) {
        if (!err) {
            ...
            res.status(500).send("Sorry, unable to upload poster image at this time (" + err.message + ")");
        }
    });
};
    ...
    if (req.body.login) { // login request
        ...
        User.findOne({
            ...
            if (user !== null) {
                ...
                res.status(500).send("Unable to login at this time; please try again later " + err.message);
            }
        });
    } else { // logout request
